...
> [!IMPORTANT]
> ### Репозиторий zapret-discord-youtube временно заблокирован
> **10 июля** аккаунт Flowseal на GitHub был временно заблокирован. Вместе с этим был заблокирован репозиторий zapret-discord-youtube.
> В это время можно использовать [ветку `reupload`](https://github.com/sch-izo/shizapret/tree/reupload) из этого репозитория. **Пользуйтесь теми источниками, которым вы доверяете**.
...
> [!IMPORTANT]
> Все файлы в папке [`bin`](./bin) скачиваются из [zapret-win-bundle/zapret-winws](https://github.com/bol-van/zapret-win-bundle/tree/master/zapret-winws) и не модифицируются при установке. Вы можете это проверить с помощью хэшей/контрольных сумм.
...
3. Распакуйте содержимое архива по пути, который не содержит кириллицу (русские буквы)/специальные символы.

4. Запустите [`shizapret.bat`](./shizapret.bat). Файл за вас все установит и обновит.

5. Рекомендуется включить Secure DNS:
...
6. Рекомендуется часто обновлять файлы с помощью [`service.bat`](./service.bat), нажав 7.

## ℹ️Краткие описания файлов

- [**`shizapret.bat`**](./shizapret.bat) - запуск стратегии вручную

  Запуск вручную можно использовать для проверки работоспособности стратегии. Работоспособность стратегии зависит от многих факторов. **Пробуйте разные стратегии (ALT, FAKE и другие), пока не найдёте рабочее для вас решение**
...
- [**`service.bat`**](./service.bat) - установка в автозапуск и другие функции:
  ...
  - **`Game Filter`** - переключение режима обхода для игр (и других сервисов, порты выше 1023).
  **После переключения требуется перезапуск стратегии.**  
  В скобках указан текущий статус (`TCP`, `UDP`, `TCP and TCP` или `disabled`).
  ...
  В скобках указан текущий статус:
    - `none` - никакие айпи не попадают под фильтр
...
### После запуска скрипта `shizapret` ничего не происходит
...
Если этого не произошло, то см. [Flowseal#522](https://github.com/Flowseal/zapret-discord-youtube/issues/522)
...
> Если стратегия вам не помогает, то вам необходимо создать новую, взяв за основу одну из представленных здесь и изменив её параметры.
...
- См. [Flowseal#765](https://github.com/Flowseal/zapret-discord-youtube/issues/765)
...
- Скачайте последнюю версию [со страницы релизов](https://github.com/sch-izo/shizapret/releases/latest) (`shizapret-...`)
...
- Если стратегия работает, можете поставить её на автозапуск: `service.bat` -> `Install Service` -> выбираете нужную
...
- См. также [Flowseal#251](https://github.com/Flowseal/zapret-discord-youtube/discussions/251)
...
- Если Discord и в браузере не работает, убедитесь что вы настроили Secure DNS, и после этого ещё раз попробуйте стратегии. Бывает такое, что на некоторых стратегиях YouTube работает, а Discord нет.
- См. также [Flowseal#252](https://github.com/Flowseal/zapret-discord-youtube/discussions/252)

### Не нашли своей проблемы

* Создайте её [тут](https://github.com/sch-izo/shizapret/issues/new/choose)
